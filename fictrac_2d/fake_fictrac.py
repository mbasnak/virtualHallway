...
class RandomFly(object):
    ...
    def get_new_angle(self):
        deltaheading = random.uniform(-self.param['angle_delta'], self.param['angle_delta'])
        return self.angle + deltaheading

    def update(self):
        new_speed = self.get_new_speed()
        new_angle = self.get_new_angle()
        self.delta_angle = new_angle - self.angle
...
class FakeFicTrac:
    ...
    def __init__(self,param=default_param):
        ...
        self.redis_client.publish('fictrac', 'test')
        self.frame = 0

    def publish_msg(self,msg):
        msg_json = json.dumps(msg)
        self.redis_client.publish('fictrac', msg_json)
    ...
    def run(self):
        ...
        while True:
            ...
            print('deltaheading:    {0}'.format(radToDeg(fly.delta_angle)))
